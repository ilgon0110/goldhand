...
export function isFormatBulletList(event: KeyboardEvent): boolean {
  const { code } = event;
  return (code === 'Numpad4' || code === 'Digit4') && isModifierMatch(event, { ...CONTROL_OR_META, altKey: true });
}

export function isFormatNumberedList(event: KeyboardEvent): boolean {
  const { code } = event;
  return (code === 'Numpad5' || code === 'Digit5') && isModifierMatch(event, { ...CONTROL_OR_META, altKey: true });
}

export function isFormatCheckList(event: KeyboardEvent): boolean {
  const { code } = event;
  return (code === 'Numpad6' || code === 'Digit6') && isModifierMatch(event, { ...CONTROL_OR_META, altKey: true });
}
...
export function isFormatQuote(event: KeyboardEvent): boolean {
  const { code } = event;
  return code === 'KeyQ' && isModifierMatch(event, { ...CONTROL_OR_META, altKey: true });
}
...
export function isStrikeThrough(event: KeyboardEvent): boolean {
  const { code } = event;
  return code === 'KeyS' && isModifierMatch(event, { ...CONTROL_OR_META, shiftKey: true });
}
