...
> 산후도우미 서비스 "고운황금손" 웹 노후화 개편 프로젝트 입니다. [nicegoldhand.com](https://nicegoldhand.com/)

SEO·Core Web Vitals·접근성·무중단 배포를 서비스의 최우선 요소로 다뤘습니다.
"했다"가 아니라 Before/After 지표로 증명하는 것을 목표로 하였습니다.

## 핵심 지표 정리

| 영역 | 결과 | 근거 |
| --- | --- | --- |
| **이미지 용량** | 업로드 이미지 용량 **99% 감소** (썸네일 기준) | [`optimizeImage.ts`](src/entities/image/lib/optimizeImage.ts) · [`generateThumnailImage.ts`](src/entities/image/lib/generateThumnailImage.ts) |
| **LCP** | 후기/이벤트 페이지 LCP **약 70% 단축** | Chrome DevTools 사용 |
| **번들** | RSC·server function 전환으로 **44% 감소** | [`app/layout.tsx`](app/layout.tsx) · `ANALYZE=true yarn build` |
| **Lighthouse** | 주요 페이지 성능 95 / SEO 100 / 접근성 94~100 | [`docs/a11y/metrics.md`](docs/a11y/metrics.md)로 1차 개선 후 Chrome DevTools로 추가 검증 |
| **배포** | 헬스체크 실패 시 **이전 버전으로 자동 롤백** (무중단) | [`scripts/deploy.sh`](scripts/deploy.sh) |
| **테스트** | MSW 기반 **128개** 테스트 (비즈니스 로직 위주) | [`__test__/`](__test__) |
...
> FSD 표준의 pages 레이어는 App Router의 app/과 역할이 충돌하므로 제거한 후 app/로 통일하였습니다.
...
- Google Search Console, Naver Search Advisor 등록 후 지속적으로 리포트 기반 SEO 향상 작업 진행 중
