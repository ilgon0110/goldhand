...
export const EventDetailPage = ({ data, docId, userData, viewCountData }: TEventDetailPageProps) => {
  ...
  useScreenView(`event_detail_${docId}`, 'EventDetailPage', { doc_id: docId });
  ...
  return (
    <>
      ...
      {isOwner && (
        <div className="flex w-full justify-end space-x-4">
          <Button
            ...
            onClick={e => {
              e.stopPropagation();
              setEventUpdateAlertDialogOpen(true);
            }}
          >
            수정하기
          </Button>
          <Button
            ...
            onClick={e => {
              e.stopPropagation();
              setEventDeleteAlertDialogOpen(true);
            }}
          >
            삭제하기
          </Button>
      ...
      <MyAlertDialog
        ...
        handleDeletePostClick={onHandleEventDeleteActionClick}
        isPending={isEventDeleteSubmitting}
        okButtonText={'삭제하기'}
        opOpenChange={open => setEventDeleteAlertDialogOpen(open)}
        open={eventDeleteAlertDialogOpen}
        title={'게시글을 삭제하시겠습니까?'}
      />
    </>
  );
